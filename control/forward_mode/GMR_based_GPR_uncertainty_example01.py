...
if __name__ == '__main__':
    ...
    # Parameters
    nb_obs = 3  # The user can change the number of observation to 0, 2 and 3.
    ...
    # GPR prediction
    mu_gp, sigma_gp = m_obs.predict(Xtest,
    ...
    # GMM
    plot_gmm(np.array(gmr_model.mu), np.array(gmr_model.sigma), alpha=0.3, color=[0.1, 0.34, 0.73], zorder=10)
    ...
    plt.plot(Xt[:, 0], mu_gmr[:, 0], color=[0.20, 0.54, 0.93], linewidth=1)
    plt.scatter(X_obs, Y_obs, color=[0, 0, 0], zorder=60, s=120)
    plt.axis('equal')
    axes = plt.gca()
